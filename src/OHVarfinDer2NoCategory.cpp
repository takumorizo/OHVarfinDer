...
OHVarfinDer2NoCategory::OHVarfinDer2NoCategory(BamReader &_tumorBamReader,
...
MutationCallResult OHVarfinDer2NoCategory::call(const CandidateWindow &window) {
    // prepare result
    ...
    if(ignoreBFComputation){
        LOG(logINFO) << "ignore_BF_Computation and return ";
    ...
    try {
        ...
        bool isHaplotypeGoodState = checkHaplotypeGoodState(window, chr, tmpTumorReads, tmpNormalReads, params, result);
            ...
            std::vector<std::vector<int> > tumorReadTypes  = OHVarfinDer2NoCategory::setReadTypes(closeVariantsForFetchingReads, window.target, tumorReads);
            std::vector<std::vector<int> > normalReadTypes = OHVarfinDer2NoCategory::setReadTypes(closeVariantsForFetchingReads ,window.target, normalReads);
            ...
            try {
                ...
                params.OHVarfinDParams2NoCategory.print();
                OHVarfinDer2Model ohvarModel = OHVarfinDer2Model(tumorLiks, normalLiks, tumorReadTypes, normalReadTypes, params.OHVarfinDParams2NoCategory,500,0.00001);
                OHVarfinDer2Model::Result ans = ohvarModel.estimate();
                if(ans.st == OHVarfinDer2Model::SUCCESS){
                    double BF =  log(getPriorRatio(window)) + ans.value;
                    ...
                    LOG(logERROR) << "something wrong during calculation of bayes factor exception" << std::endl;    
                    throw std::string("something wrong during calculation of bayes factor");
                }
                LOG(logINFO) << "end check BF " << std::endl;
...
void OHVarfinDer2NoCategory::setTNReads(std::string chr, int pos,
...
bool OHVarfinDer2NoCategory::checkHaplotypeGoodState(const CandidateWindow &window, std::string chr, 
...
bool OHVarfinDer2NoCategory::checkHaplotypeBuildPossibilityAndBuildHaplotype(std::vector<Haplotype> &normalHaps, std::vector<Haplotype> &refHaps, MutationCallResult &result,
                                                                   std::string chr, 
...
bool OHVarfinDer2NoCategory::checkHaplotypeBuildPossibilityAndBuildSomaticHaplotype(std::vector<Haplotype> &haps, std::vector<Haplotype> &haps_ref,
...
void OHVarfinDer2NoCategory::buildHapPaires(std::vector<std::pair<Haplotype, Haplotype> > &hapPairs, 
                                  std::vector<std::pair<Haplotype, Haplotype> > &hapPairsBasic,
...
bool OHVarfinDer2NoCategory::isBFWithHapComputable(bool isHaplotypeGoodState, HaplotypeBuilder::State state, MutationCallResult &result){
