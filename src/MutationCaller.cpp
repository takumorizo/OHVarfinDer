...
MutationCallResult MutationCaller::call(const CandidateWindow &window) {
    ...
    try {
        ...
        if (ignoreBFComputation == true) {
            ...
            std::vector<std::vector<double> > tumorLiksForBasicTmp = ProfileHMMUtils::calcBasicLikelihoods(hapsForBasicTmp, tumorReads,   pos);
            std::vector<std::vector<double> > normalLiksForBasicTmp = ProfileHMMUtils::calcBasicLikelihoods(hapsForBasicTmp, normalReads, pos);
