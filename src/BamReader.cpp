...
BamReader::BamReader(const MyBam *_bam, const faidx_t *_fai, Parameters param,
                     int _maxReads) : fFlag(param.fReadFilter), FFlag(param.FReadFilter) {
    fai = _fai;
    bam = _bam;
...
std::vector<IRead *> BamReader::getPairedReads(std::string chr,
    ...
    LOG(logINFO) << "FFlag :" << FFlag << ", fFlag :" << fFlag  << std::endl;
    std::map<std::string, std::list<int> > seqNameHash; // query name to read idx
    std::map<std::string, std::list<int> >::const_iterator hashIt;
        for (size_t r = 0; r < readBuffer.size(); r++) if( readBuffer[r]->hasFilter(fFlag) && readBuffer[r]->notHasFilter(FFlag) ) {
        if (!readBuffer[r]->isUnmapped()) {
            ...
            if (seqNameHash[std::string(bam1_qname(readBuffer[r]->getBam()))].size() > 2) {
                ...
                LOG(logERROR) << std::string(bam1_qname(readBuffer[r]->getBam())) << std::endl;
    ...
    for (int r = 0; r < int(readBuffer.size()); r++) if( readBuffer[r]->hasFilter(fFlag) && readBuffer[r]->notHasFilter(FFlag) ){
