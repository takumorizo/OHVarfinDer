...
double MutationModel::estimate() {
    ...
    LOG(logINFO) << "estimated parameters" << std::endl;
    LOG(logINFO) << "ahat:";
    printVec(ahat);
    ...
    LOG(logINFO) << " model @ " << estimationType << std::endl;
    LOG(logINFO) << "===== EZ_tumor logging : ====="  << std::endl;     
    for(int i = 0; i < zt.size()/4; i++){
        LOG(logINFO) << " get EZ info " << std::endl; 
        for(int j = 0; j < 4; j++){
            LOG(logINFO) << "zt[" << i << "][" << j << "] : " << zt[i*4+j] << std::endl; 
        }
    }
